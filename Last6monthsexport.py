...
output_csv = "channel_history_last_6months.csv"
...
# Function to export channel history to CSV for the last 6 months
def export_last_6months_channel_history_to_csv():
    try:
        with open(output_csv, 'w', newline='') as csv_file:
            ...
            result = client.conversations_history(channel=channel_id, oldest=six_months_ago_timestamp)
                ...
                for message in result["messages"]:
                    ...
                    for attachment in attachments:
                        if attachment.get("image_url"):
                            image_url = attachment["image_url"]
                            image_filename = os.path.join(output_images_folder, f"{timestamp}.jpg")
                            download_and save_image(image_url, image_filename)
                            csv_writer.writerow([user, timestamp, text, image_filename])
                        else:
                            csv_writer.writerow([user, timestamp, text, ""])
...
if __name__ == "__main__":
    export_last_6months_channel_history_to_csv()
